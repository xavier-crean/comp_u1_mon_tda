...
def parseConfig(config_buffer,HL,L):
    ...
    config_array = np.array([[[[[cNumFromBytes(config_buffer[HEAD_LEN+DTYPE_LEN*(STDIM*(t+(x*DSIZE_T)+(y*DSIZE_T*DSIZE_X)+(z*DSIZE_T*DSIZE_X*DSIZE_Y))+link):HEAD_LEN+DTYPE_LEN*(STDIM*(t+(x*DSIZE_T)+(y*DSIZE_T*DSIZE_X)+(z*DSIZE_T*DSIZE_X*DSIZE_Y))+link+1)])
...
def wilsonLoop(conf, t, x, y, z, directions):
        ...
        ang = (
                ...
                +confang[(t+d_mu[0][0])%conf.shape[0],(x+d_mu[0][1])%conf.shape[1],(y+d_mu[0][2])%conf.shape[2],(z+d_mu[0][3])%conf.shape[3],directions[1]]
                -confang[(t+d_mu[1][0])%conf.shape[0],(x+d_mu[1][1])%conf.shape[1],(y+d_mu[1][2])%conf.shape[2],(z+d_mu[1][3])%conf.shape[3],directions[0]]
