...
if os.path.exists(filename):
    ...
    # Load persistence diagram HDF5 files, compute Betti numbers and save as HDF5 file
    pd = [
        [
            h5py.File(
    ...
    h = np.array(
        [
            [multiplicity_total(pd[l][b]) for b in range(len(betas[l]))]
            for l in range(len(Ls))
        ]
    )
...
b0_bootstrap = np.array(
    [
        [
            np.array(
                [h[l, :, :, 0][b, bootstraps[l][b][i]] for b in range(len(betas[l]))]
            )
            for l in range(len(Ls))
        ]
        for i in range(N_bootstraps)
...
b1_bootstrap = np.array(
    [
        [
            np.array(
                [h[l, :, :, 1][b, bootstraps[l][b][i]] for b in range(len(betas[l]))]
            )
            for l in range(len(Ls))
        ]
        for i in range(N_bootstraps)
...
# Load action HDF5 files and compute $\Delta S$

# Load action HDF5 files
s = np.array(
    [
        [
            h5py.File(
...
def reweight(O, E, rw_betas, l):
    ...
    A = np.concatenate(O)
    mean = mbar.compute_expectations(A, u_kn=potentials, compute_uncertainty=False)[
        "mu"
    ]
...
for l in range(len(Ls)):
    ...
    if os.path.exists(filename):
        ...
        rw_means_linear_temp, rw_vars_linear_temp = reweight(
            s[l] / (6 * (Ls[l] ** 4)), delta_s[l], rw_betas_linear[l], l
        )
        with h5py.File(filename, "w") as hf:
...
for l in range(len(Ls)):
    ...
    if os.path.exists(filename):
        ...
        rw_means_linear_temp, rw_vars_linear_temp = reweight(
            h[l, :, :, 0] / (Ls[l] ** 4), delta_s[l], rw_betas_linear[l], l
        )
        with h5py.File(filename, "w") as hf:
            hf.create_dataset("rw_means_linear", data=rw_means_linear_temp)
            hf.create_dataset("rw_vars_linear", data=rw_vars_linear_temp)
...
for l in range(len(Ls)):
    ...
    if os.path.exists(filename):
        ...
        rw_means_linear_temp, rw_vars_linear_temp = reweight(
            h[l, :, :, 1] / (Ls[l] ** 4), delta_s[l], rw_betas_linear[l], l
        )
        with h5py.File(filename, "w") as hf:
            hf.create_dataset("rw_means_linear", data=rw_means_linear_temp)
            hf.create_dataset("rw_vars_linear", data=rw_vars_linear_temp)
...
pseudo_bcs_E = np.array(
    [rw_betas_linear[l][np.argmax(rw_vars_linear_E[l])] for l in range(len(Ls))]
)
pseudo_bcs_b0 = np.array(
    [rw_betas_linear[l][np.argmax(rw_vars_linear_b0[l])] for l in range(len(Ls))]
)
pseudo_bcs_b1 = np.array(
    [rw_betas_linear[l][np.argmax(rw_vars_linear_b1[l])] for l in range(len(Ls))]
)
...
delta_s_bootstrap = np.array(
    [
        [
            np.array([delta_s[l][b][bootstraps[l][b][i]] for b in range(len(betas[l]))])
            for l in range(len(Ls))
        ]
        for i in range(N_bootstraps)
    ]
...
E_bootstrap = np.array(
    [
        [
            np.array(
                [
                    (s[l] / (6 * (Ls[l] ** 4)))[b, bootstraps[l][b][i]]
                    for b in range(len(betas[l]))
                ]
            )
            for l in range(len(Ls))
...
for l in range(len(Ls)):
    ...
    if not os.path.exists(filename):
        ...
        for i in range(N_bootstraps):
            rwm, rwv = reweight(
                E_bootstrap[i][l], delta_s_bootstrap[i][l], rw_betas_linear[l], l
            )
...
for l in range(len(Ls)):
    ...
    if not os.path.exists(filename):
        ...
        for i in range(N_bootstraps):
            rwm, rwv = reweight(
                b0_bootstrap[i][l], delta_s_bootstrap[i][l], rw_betas_linear[l], l
            )
...
for l in range(len(Ls)):
    ...
    if not os.path.exists(filename):
        ...
        for i in range(N_bootstraps):
            rwm, rwv = reweight(
                b1_bootstrap[i][l], delta_s_bootstrap[i][l], rw_betas_linear[l], l
            )
...
pseudo_bcs_E_bs = np.array(
    [
        [
            rw_betas_linear[l][np.argmax(rw_vars_linear_E_bs[i][l])]
            for l in range(len(Ls))
        ]
        for i in range(N_bootstraps)
    ]
...
pseudo_bcs_b0_bs = np.array(
    [
        [
            rw_betas_linear[l][np.argmax(rw_vars_linear_b0_bs[i][l])]
            for l in range(len(Ls))
        ]
        for i in range(N_bootstraps)
    ]
...
pseudo_bcs_b1_bs = np.array(
    [
        [
            rw_betas_linear[l][np.argmax(rw_vars_linear_b1_bs[i][l])]
            for l in range(len(Ls))
        ]
        for i in range(N_bootstraps)
    ]
...
with open(filename, "w") as f:
    data = [
        (
            format_NIST(pseudo_bcs_E[l], yerr_E[l]),
            format_NIST(pseudo_bcs_b0[l], yerr_b0[l]),
            format_NIST(pseudo_bcs_b1[l], yerr_b1[l]),
        )
        for l in range(len(Ls))
    ]
    tab_content = r"""
...
results = [
    [
        fss_rchi2(type, range_[i], d, ps_bc, ps_bc_bs)
        for i in range(len(range_))
        for d in range(1, len(range_[i]) - 1)
    ]
    for type, ps_bc, ps_bc_bs in [
        ("E", pseudo_bcs_E, pseudo_bcs_E_bs),
        ("rho_b0", pseudo_bcs_b0, pseudo_bcs_b0_bs),
        ("rho_b1", pseudo_bcs_b1, pseudo_bcs_b1_bs),
    ]
]
